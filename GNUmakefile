...
MACHSTATS := \
   ...
   DOn:em64t:linux:gnu linx64:em64t:linux:intel linx32:pentium4:linux:intel \
   crunchb:native64:sunos:forte \
   solx:x86:sunos:pg \
   submit1:x86_64:linux:intel\
   r7a-s1:x86_64:linux:intel\
...
ifeq (${CMPLR},intel) ##########################################################
   ...
   LD   = ifort
 # make sure the other compilers f9x, cc, and CC all get double alignment
   CFLAGS   = -Zp8 #-xSSE2 -axSSE4.2,SSE4.1,SSSE3,SSE3
   F9XFLAGS = -Zp8 -zero #-xSSE2 -axSSE4.2,SSE4.1,SSSE3,SSE3
   LDFLAGS  = -Zp8
   ifneq (,$(findstring ${ARCH},em64t itanium2 x86_64))
      64BIT = 1
      F9XFLAGS += -i8
      LDFLAGS  += -i8
   endif
